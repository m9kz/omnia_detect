...
from app.domain.ports.repositories.dataset import DatasetRepository
from app.domain.ports.repositories.model import ModelRepository
...
class SqlAlchemyUnitOfWork:
    def __init__(self, db_url: str):
        ...
        self.datasets = _DatasetRepo(self)
        self.models = _ModelRepo(self)
...
class _ModelRepo(ModelRepository):
    ...
    def list(self, limit: int = 50):
        s = self.uow._session
        q = s.query(ModelRow).order_by(ModelRow.created_at.desc()).limit(limit)
        for r in q:
            yield ModelArtifact(
                id=UUID(r.id),
                dataset_id=UUID(r.dataset_id),
...
class _DatasetRepo(DatasetRepository):
    def __init__(self, uow: SqlAlchemyUnitOfWork):
        self.uow = uow
    ...
    def get(self, dataset_id: UUID) -> DatasetArtifact | None:
        s = self.uow._session
        r = s.get(DatasetRow, str(dataset_id))
        if not r: return None
    ...
    def list(self, limit: int = 50):
        s = self.uow._session
        q = s.query(DatasetRow).order_by(DatasetRow.created_at.desc()).limit(limit)
        for r in q:
